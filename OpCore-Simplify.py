...
from Scripts import linux_gatherer
...
class OCPE:
    def __init__(self):
        ...
        self.r = run.Run()
        self.l = linux_gatherer.LinuxGatherer() if os.name != "nt" else None
    ...
    def select_hardware_report(self):
        ...
        while True:
            ...
            user_input = self.u.request_input("Drag and drop your hardware report here (.JSON){}: ".format(" or type \"E\" to export"))
            ...
            if user_input.lower() == "e":
                ...
                if os.name == "nt":
                    ...
                    report_data = self.l.gather_report()
                    self.u.write_file(report_path, report_data)
                    self.l.dump_acpi(acpitables_dir)
