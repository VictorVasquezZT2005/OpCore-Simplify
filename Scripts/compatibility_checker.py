...
class CompatibilityChecker:
    ...
    def check_cpu_compatibility(self):
        ...
        # If SSE4 is present in any form (SSE4.1 or SSE4.2)
        ...
        # Special case for Celeron/Pentium: they are technically unsupported but we allow them
        if self.is_low_end_intel_cpu(self.hardware_report.get("CPU").get("Processor Name")):
            if max_version is None:
                # Force some support if we have at least SSE4
                max_version = "17.99.99" # High Sierra
                min_version = "17.0.0"
    ...
    def check_gpu_compatibility(self):
        ...
        if self.hardware_report.get("GPU"):
        ...
        if max_supported_gpu_version == min_supported_gpu_version and max_supported_gpu_version == None:
            print("")
            print("\033[1;93mWarning:\033[0m No supported GPU found for macOS.")
            print("You may still proceed, but you will NOT have graphics acceleration (VESA mode).")
            print("This will result in a very laggy and poor experience.")
            print("")
            self.utils.request_input("Press Enter to continue anyway...")
            # Set a dummy version range to allow the program to continue
            max_supported_gpu_version = os_data.get_latest_darwin_version()
            min_supported_gpu_version = os_data.get_lowest_darwin_version()
